...
contract CDH is ICDH, ERC721, ERC721Enumerable, ERC721Burnable {
	...
	// tokenId => tbaAccount
	mapping(uint256 => address) public tokenIdToTbaAccount;
	// EOA address => tbaAccount
	mapping(address => address) public eoaToTbaAccount;
	...
	function safeMint(address to, string memory uri) public notOwnNFT(to) {
		...
		address tbaAccount = IERC6551Registry(erc6551Registry).createAccount(erc6551Account, bytes32(tokenId), 5555, address(this), tokenId);
	...
	function getOwners(uint256 amount) public view returns (address[] memory) {
		address[] memory owners = new address[](	function getOwners(uint256 amount) public view returns (address[] memory) {
);
		for (uint256 i = 0; i < fieldOwners.length; i++) {
			owners[i] = fieldOwners[i];
		}
		return owners;
	}
