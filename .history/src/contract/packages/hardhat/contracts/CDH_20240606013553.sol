...
function getOwners(uint256 amount, uint256 from) public view returns (address[] memory) {
    uint256 total = totalSupply();
    if (from >= total) {
        return new address ; // If 'from' is greater than or equal to total supply, return an empty array
    }
