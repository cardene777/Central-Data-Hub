...
contract CDH is ICDH, ERC721, ERC721Enumerable, ERC721Burnable {
	...
	function getFields(uint256 amount, uint256 from) public view returns (string[] memory) {
		uint256 total = fieldNames.length;
		if (from >= total) {
			return new string[](0);
		}

		uint256 validAmount = (from + amount > total) ? total - from : amount;
		string[] memory fields = new string[](validAmount);
		for (uint256 i = 0; i < validAmount; i++) {
			fields[i] = fieldNames[from + i];
		}
		return fields;
	}
