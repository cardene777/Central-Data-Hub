...
contract CDH is ICDH, ERC721, ERC721Enumerable, ERC721Burnable {
	...
	/// @notice get owners and their corresponding TBA accounts
/// @param amount amount
/// @param from starting index
function getOwners(uint256 amount, uint256 from) public view returns (address[] memory, address[] memory) {
    uint256 total = totalSupply();
    if (from >= total) {
        return (new address , new address ); // If 'from' is greater than or equal to total supply, return an empty array
    }
    ...
    for (uint256 i = 0; i < validAmount; i++) {
        ...
        tbaAccounts[i] = eoaToTbaAccount[owners[i]];
    }
